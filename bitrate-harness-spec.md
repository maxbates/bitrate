...
**Word-level targets — position refined (2026-07-22).** i.i.d. uniform sampling from a *fixed, versioned word list* is rule-1 compliant: it is simply an alphabet whose symbols are words. What stays banned is predictive assistance and non-uniform sampling; the earlier blanket "ban" conflated the two. The economics still say words don't help *typing*: a word costs its letters, so a 1000-word list of 3–4 letter words gives log2(999) ≈ 9.96 bits at ~1.8 words/sec ≈ 18 bps, versus ~19 bps for random characters — a near-perfect wash, now measured by the `word-typing` environment rather than assumed. **For speech the algebra flips**: one utterance = one selection regardless of word length, so a large word alphabet multiplies bits per utterance (`speech-words`, §5). Word lists must be bundled and versioned — device dictionaries vary by machine and would break content-addressed variant identity.
...
- `word-typing` — **built.** i.i.d. words from the bundled `common-v1` list (2–5 letters, max length a setting), typed; selection = committed word (space commits, backspace edits the buffer freely and is not a selection; N = word count). Exists to measure §2.2's predicted wash honestly. Server untouched — words ride the numeric-alphabet machinery with client-side index↔word mapping.
- `speech-words` — **built.** i.i.d. words from the homophone-pruned speech list, spoken; one utterance = one selection, so the alphabet size (256/512/1024/all ≈ 1.7k) multiplies bits per utterance — the word-alphabet trick is structurally right for voice. Recognition = Web Speech API: on-device where available (Chrome `processLocally`, macOS dictation), else the browser's default engine **which may use a network service — surfaced in-UI; lab-only** (§6 binds anything shipping; the offline path would be vendored WASM ASR per §4.1). The recognizer's language model lives in the recognition channel; target generation stays i.i.d. (rule 1). Recognizer constructed only on a trusted user gesture (backend-less headless builds hard-crash on construction; mic permission wants a gesture anyway).
...
- `beat-hands` — Beat Saber-style: upcoming targets stream toward the player, each specifying hand (left/right) x pose angle (4 angles), read by webcam — N=16, log2(15) ≈ 3.9 bits/event. Deliberately violates self-pacing: presentation speed is fixed per run (v1: a config value; v2: written by a calibration pre-step per §2.6). Tests whether deep-lookahead paced execution can beat reactive self-pacing.
