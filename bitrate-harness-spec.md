...
- `pixel-lens` — **built.** A massive grid where a target lights up, the player mouses to it and clicks, with a loupe magnifying around the cursor. Huge N, but Fitts's law charges log-distance per acquisition — expect the pointer bottleneck (~4–10 bps) to dominate no matter how large N grows. Worth building to show *why* alphabet size can't rescue a serial pointing device. Implementation decisions:
  ...
  - **Touch/mouse input axis + look-ahead previews (built 2026-07-23; a third mode `hybrid` and a v2 preview grammar landed 2026-07-24 — see the bullets below).** `input: mouse | touch` is now part of the config (so it is part of the variant identity), which makes direct-touch vs indirect-mouse a within-environment leaderboard comparison instead of two separate environments. Touch **drops the loupe** — a touchscreen has no hover to magnify around before committing, so you tap the target directly — and switches to **finger-sized cells** (8/10/12/16/20/25 mm — the 8/10 pair added 2026-07-24, see the sub-finger bullet below — vs the mouse 3/5/7.5/10); Pointer Events unify both input paths, and touch feedback is a ring that pops at the tap point (no loupe rim to flash). `preview: 0–3` shows that many upcoming targets as **dimmer green bull's-eyes, visually distinct from the solid-yellow fill of the live cell** (color = "next" vs "now", a functional affordance, not symbol encoding) — **the pointing analog of stream-typing's visible character stream**: non-interactive (a tap only ever resolves the live target; tapping a preview cell just consumes the current target as an incorrect selection under advance-always), so it leaks no bits — targets stay i.i.d. uniform over N — yet lets the player pre-plan the next saccade + reach (eye-hand span), a legitimate throughput gain. Each `input`/`preview`/cell combination is its own content-addressed variant, so look-ahead's value is directly measurable. Server unchanged (config is opaque, canonical-JSON hashed; `alphabet_size` still drives N). pixel-lens uses no camera, so it runs over **plain HTTP on the LAN** (no getUserMedia secure-context requirement) — an iPad on the same WiFi reaches `-addr :4700`, and the server now prints the reachable LAN URLs at startup. This realizes the `touch-grid` sketch below as pixel-lens config axes rather than a new environment. Headless-testable via `window.pixelDebug` (state/config/counts/targetCell/previewCount/tapCell/trailingBps/sparkSeries). A `mouse`-with-`loupe:off` variant to isolate the loupe's contribution is a possible future axis (not built — input mode currently drives loupe presence).
  - **Sub-finger touch cells: 8 mm and 10 mm added to the touch axis (2026-07-24, owner request).** The touch list is now 8/10/12/16/20/25 mm (default still 20). These two sit *below* finger-contact width (~9 mm pad), which the §5 `touch-grid` estimate calls the point where "error and search costs bend the curve down" — that is exactly why they exist. The owner's 2026-07-24 touch runs were a flat speed–information plateau (N=84 @ 2.50 taps/s vs N=275 @ 2.12 taps/s, both ~15 bps), and the fitted IKI = 100 ms + 44 ms/bit says gross saturates ~23 bps as N→∞. Those two points bracket a regime where cells were still comfortably ≥ a fingertip; 8/10 mm probes the other side of the knee, where the tap itself starts to fail. Measured at an 11″ iPad viewport (1194×834, field 1194×668) the touch ladder is now 8 mm → 39×21 = N 819 (**9.68 bits/tap**), 10 → 30×17 = 510 (**8.99**), 12 → 25×14 = 350 (8.45), 16 → 19×10 = 190 (7.56), 20 → 15×8 = 120 (6.89), 25 → 12×6 = 72 (6.15) — so if accuracy holds anywhere near the 94–97 % the larger cells got, net bps rises; if it collapses, the advance-always Sc−Si penalty prices that honestly and the plateau claim is confirmed from above rather than assumed. Either outcome is a result, and each size is its own content-addressed variant, so the comparison is a leaderboard read, not a re-analysis. Zero mechanism change: cells are already viewport-derived and everything (`#target .fill`, the grid lattice, the touch grid inset `pad = cell/2`) scales off `cell_px`; the sheet's cell-size segment now wraps (six options overflow a phone-width row) and its buttons carry a 32 px min hit height. Mouse options are untouched — 10 mm exists in both lists, and because `input` is part of the config the two 10 mm variants stay distinct, which incidentally gives a **same-cell-size direct-touch vs indirect-mouse pair** the axis didn't previously have.
  - **Look-ahead rank encoding v2 — form, not opacity (2026-07-24, owner report: "just using opacity is not enough… it's hard to predict quickly which the next will be").** v1 drew every preview as the same green bull's-eye and ranked them by opacity alone. On device that showed you the *set* but not the *order* fast enough to pre-plan, which is the only reason look-ahead exists — and it is the same failure the `lane-tap` entry credits for the aborted preview-3 bouts ("the vertical stacking makes lookahead **order** self-evident — the thing the 2D preview dots couldn't show"). Since pixel-lens is 2D and cannot stack, v2 gives each rank its own **form**, plus a drawn path:
    1. **next = a filled green tile the size of its cell** — deliberately the same *class of object* as the live yellow tile (same inset, same corner radius, no glow), so it reads as "the cell you act on after this one" instead of as one dot among several. This is the owner's own suggestion ("a different color tile for first look ahead").
    2. **rank 2 = an open ring** (hollow never reads as filled), **rank 3+ = a solid disc**, each smaller and fainter than the last.
    3. **a dashed path joins live → next → … in sequence**, trimmed to stop short of each mark so it never draws through one (the untrimmed first cut read as a stray arrowhead inside the tile). Segments fade with depth, so the path reads directionally too.
    Why form rather than a bigger size ramp: at an 8 mm cell (30 px) a pure size ramp is a few pixels per step, so the cue that was supposed to rescue opacity dies exactly in the new small-cell variants. Filled-tile → ring → disc survives both small cells and dimming. **Rank order is not symbol information** — the previews are the same i.i.d. targets shown early, so nothing leaks; color still only ever says now/next. The same grammar is redrawn inside the loupe (`drawScene`), because the lens must not teach a different visual language than the field behind it — and in hybrid mode the lens is where the player actually looks. The grammar is part of the task, so `preview_style: 'rank-tile-path-v2'` joins the config **only when previews are on**: preview-off variants keep their existing hash and their existing leaderboard history, while preview-on v1 runs stay separated from v2 runs instead of being silently pooled.
  - **`input: hybrid` — touch with the loupe, resolved by take-off selection (2026-07-24, owner request).** A third input mode: **finger-down summons and drags the lens, and the LIFT is the selection.** This is the missing middle of the axis. `touch` had to drop the loupe because a touchscreen has no hover — but *press* is a hover if you don't treat it as a commit, so hybrid recovers pre-commitment aiming on a tablet: the finger can settle onto a cell narrower than its own contact patch, watching the magnified crosshair, and only then let go. It is the direct answer to whether the sub-finger 8/10 mm cells above are hittable at all, and it is the classic land-on vs take-off pointing comparison run inside our own accounting. Decisions:
    - **The lens is drawn offset above the contact point**, not centred on it (`loupeAnchor`): a lens centred under the fingertip magnifies precisely the spot the finger is covering, which is the entire problem it exists to solve. The magnified content still centres on the contact point and the crosshair still marks it, so it reads like the iOS text loupe. Clamped inside the field so it never leaves the screen; near the top edge that means it slides back over the finger, the least-bad option there.
    - **Cell options are identical to `touch`** (8/10/12/16/20/25, default 12 — finer than touch's 20, since the lens is the whole reason to expect a sub-fingertip cell to be hittable). Identical lists make hybrid-vs-touch a **paired comparison at every cell size**, which is the point of building it; 10 mm now exists in all three modes, giving a clean same-cell-size mouse / touch / hybrid triple.
    - **A cancelled pointer is an aborted aim, not a selection** (dropped silently), and the field takes pointer capture on press so a lift that strays over the header still lands — otherwise a drag to the edge would swallow the selection and quietly cost the player a target under advance-always.
    - **`commit: 'lift'` + `recognizer: 'touch-loupe-takeoff-v1'` in the config, emitted only for hybrid** (existing mouse/touch hashes unchanged). Pressing-to-select and lifting-to-select are different tasks — you may re-aim while pressed — so they must not share a variant. `t_pressed_ms` remains *the moment of commitment* (the lift) because IKI and pace are differences of that field. **Known limitation:** the press→lift aim duration is therefore not persisted — the server's `Selection` struct has no field for it and would drop an extra one — so the dwell-vs-travel split is only observable client-side. Recorded rather than papered over; adding it means changing the shared selection schema, which no other environment needs yet.
  - **Grid inset recovers 40 % of its padding on small screens (2026-07-24, owner request: "get at least 30 % of the padding back").** The inset was tuned for a desktop window, where a lens-radius margin is cheap; on an 11″ iPad it costs a visible share of the only scarce resource, play area. Now `pad × 0.6` whenever `min(innerWidth, innerHeight) < 900` — a threshold that lands iPad-class viewports (834) and phones on the small side while leaving a 900-tall laptop alone. Hybrid also insets like `touch` (half a cell) rather than like `mouse` (0.6 × lens radius), because its lens is offset and clamped and so never needs the lens-sized boundary margin. Consequence to be explicit about: `grid_pad_px` feeds N, so **same-setting variants get new config hashes** on small screens (an iPad touch/8 mm run went N 798 → 819). That is the normal viewport-derived variant churn this environment already has, not a scoring change — but existing small-screen variants will not extend their old leaderboard rows.
  - **Practice metrics are a trailing-60 s window + rolling sparkline — shared across EVERY environment (2026-07-23).** The practice HUD bps/counts reflect only the last 60 s so the figure tracks current skill instead of being dragged down by warm-up; a small **client-side rolling sparkline** (bits/s over a 6 s window sampled each second, blue so it never collides with target colors) sits in the **header strip, just left of the HUD text** (a positioned host anchored to `#hud` via `right:100%`), so it stays out of the play area; its x-span fills the width for the first minute then holds at 60 s and scrolls. Scored-run HUD stays **cumulative** (it previews the actual 60 s score), and the sparkline is practice-only. Because every environment logs the same `{verdict, t_pressed_ms}` selection schema, the computation and SVG are one shared implementation in `common/results.js` (`trailingBps` / `sparkSeries` / `sparkHTML`, styled by `common/results.css`), called from each env's `renderHud` — keys, pointer, voice, and swipes all feed the same code. All derived from the selection log in-browser — no server involvement. (In stream-typing, the shipped baseline, it is practice-only and cleared during the scored run, so the graded experience is unchanged.)
