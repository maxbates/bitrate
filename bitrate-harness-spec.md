...
**Open item — the header spills on iOS.** The corner cluster + `res-info` + HUD + rolling sparkline are laid out for a desktop-width top strip; on narrow/tall iPad viewports the content wraps and overflows its fixed-height band. In pixel-lens this is worst because `#topbar` is a hard `height:112px` and `#field` is pinned at `top:112px`, so any header overflow **draws over the top of the play grid** rather than pushing it down. Todo (not yet built — surface before implementing, per workflow):
  - Make the header band responsive: allow it to reflow/grow on small widths and drive `#field`'s top offset from the header's *actual* height (measured or CSS var), not a hard-coded 112 px, so the grid always starts below it.
  - On touch/narrow viewports, demote secondary chrome — the `res-info` accounting line and the sparkline are the least essential during play; collapse or hide them under a threshold (the honest-N line can live on the results screen), keeping mode banner + bps/time/counts.
  - Audit the same corner/HUD pattern in the other environments (stream-typing, voice-babble, beat-hands, twin-stick) — the sparkline host + HUD share `common/results.css`, so a shared responsive rule likely fixes several at once.
  - The gallery back-link (`← gallery`, bottom-left → `/env/`) added to twin-stick 2026-07-23 is the pattern for a consistent "escape to the gallery" affordance; propagate to the other environments when touching their chrome.
