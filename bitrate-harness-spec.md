...
- `pixel-lens` — **built.** A massive grid where a target lights up, the player mouses to it and clicks, with a loupe magnifying around the cursor. Huge N, but Fitts's law charges log-distance per acquisition — expect the pointer bottleneck (~4–10 bps) to dominate no matter how large N grows. Worth building to show *why* alphabet size can't rescue a serial pointing device. Implementation decisions:
  ...
  - **The look-ahead error is named, not just flashed (2026-07-26).** Look-ahead dots have exactly one failure mode, and it is a first-session one: reading "next" as "now". A green bull's-eye is the universal aim-here symbol and it shares the screen with the live yellow cell, so the mistake is a design consequence, not carelessness. Tapping one already drew the loudest visual in the game (a red pulse round the whole field — the tap-point burst was dropped by owner's call, see the bullet above) — but a flash says *wrong* without saying *what instead*. It now also says it: a red one-line notice, "◦ **look-ahead** dot — tap the ▪ **yellow square**", with both swatches drawn the way the game draws them (filled yellow cell, green ring) so the sentence **teaches the mapping instead of naming colors**. Same mechanism as the wrong-input warning and the same rules, because this one fires *mid-run*: `pointer-events: none` so it can never eat a tap, fixed in the footer strip the grid already leaves empty (and above the gallery link), no layout change of any kind, 3 s, timer restarts on repeats. Advance-always means the tap has already consumed the target by the time it shows, so the copy is a standing instruction and never points at a cell that has since moved. The same legend also shows **once per page load before the first tap** ("▪ yellow = tap now · ◦ green = next"), and only when look-ahead is on — the cheapest moment to say it is before the mistake. One implementation backs both games, so both get it; pixel lens defaults to look-ahead off, so in practice it is drum pad's. `pixelDebug.noticeText()` exposes it for headless assertions, alongside `earlyFlashCount()`, which counts invocations rather than DOM nodes now that there is no node to count. Scoring, ground truth, and the i.i.d. sequence are untouched: this is feedback about a selection already made.
  - **Both outcomes make a sound, and the good one is the quiet one (2026-07-26).** A miss buzzed and a hit said nothing, which makes the only audio in the game a punishment — and leaves the player's ear with no confirmation on the selection they actually want to repeat. A correct tap/click now plays a **soft ding: sine, C6 (1046.5 Hz), 55 ms, peak gain 0.01**, against the miss buzz's **square, 220 Hz, 100 ms, peak gain 0.12**. Deliberately the quieter of the two on every axis — a twelfth the amplitude, a sine's clean spectrum rather than a square's harmonics, and short enough to finish before the next tap — because a hit fires several times a second at speed and has to be something the ear can sit inside for a full 60 s, while a miss is rare and *should* intrude. The pitch gap (a little over two octaves) is what makes the two unmistakable without listening for them. Both ride the **existing `audio_feedback` boolean**: the flag widens from "error sound" to "sound" (the settings row is relabelled to match) rather than gaining a sibling key, because a new config key would mint a fresh content-addressed variant for every player over a change that touches no alphabet, no pacing, and no scoring — splitting the leaderboard for a sound. Keystroke environments are untouched for now; a tone on every correct key at 8–10 keys/s is a different question from one on every tap.
  - **Text selection is off across the play area, in both spellings (2026-07-26).** `user-select: none` was already on `body` in stream typing and pixel lens/drum pad, but **unprefixed only** — and WebKit honours `-webkit-user-select`, so on iOS Safari (the phone/iPad case these two are aimed at) a long-press still selected the stream or the grid and raised the callout menu on top of a run. Both spellings now sit on `body` and again on the play surfaces (`#field`, `#stage`), with `-webkit-touch-callout: none` and a transparent tap highlight. `#kbd-catcher` re-declares `user-select: text` for itself, since the soft-keyboard path needs a genuinely editable field.
...
**The README lives at the repo root and there is no packaging directory (2026-07-26).** `ship/` is **deleted**. It existed only to stage a ZIP, and with the bundle gone there is nothing to stage: `README.md` sits at the repo root, where it is simultaneously GitHub's landing page for the public repo and the source for `/readme`. `readme.go` embeds it, `server/readme.go` renders it through a hand-rolled markdown subset (headings, paragraphs, bullet lists, fenced code, tables, rules, inline code/bold/em/links) because Tier A is stdlib-only and no dependency is worth taking for one page — anything outside the subset degrades to a paragraph rather than breaking. `/readme.md` serves the raw bytes. **One file, two audiences, no copy to drift.** It is linked from the results card, the moment a player has a score to read it against, and from the gallery footer.

`build.sh` no longer produces a ZIP; it builds the local binary, plus the linux/amd64 one on `./build.sh deploy`. **`run.sh` at the root is now the repo's only launcher** and doubles as requirement 5's artifact — it needs Go, which is the honest cost of dropping the prebuilt-binary bundle (see §1 register item 7: the URL is the submission, `run.sh` is the included fallback).

Two things are consequently **orphaned, and deliberately not yet deleted**: the `-tags ship` profile (`server/profile_{lab,ship}.go`) has no consumer now that the deployed lab build *is* the deliverable, and `lab/ship_gate.py` has no ZIP to unzip so it is no longer invoked by `scripts/pre-push`. The gate is kept because most of what it does — drive a real scored run headlessly, assert the results card, assert server/reference score agreement, assert zero non-localhost requests — is still exactly what should be asserted, just against a running server instead of a bundle. **Repointing it is an open task; until then nothing enforces the no-external-requests property at runtime** (the static `TestNoExternalURLsInEnvironments` check remains).
