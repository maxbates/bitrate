...
- `pixel-lens` — a massive grid where every cell is one symbol; one lights up, the player mouses to it and clicks, with a fisheye lens magnifying around the cursor to keep hitboxes usable. Huge N, but Fitts's law charges log-distance per acquisition — expect the pointer bottleneck (~4–10 bps) to dominate no matter how large N grows. Worth building to show *why* alphabet size can't rescue a serial pointing device.
