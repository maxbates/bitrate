...
- `word-typing` — **built; OFF-BRIEF.** Violates rule 1's explicit "no word-level targets" (see §2.2) — can never ship, never a grading candidate, never cited in compliance-relevant comparisons; badged off-brief in the gallery. Kept as a lab toy because the measurement is still interesting: i.i.d. words from the bundled `common-v1` list, selection = committed word; tests §2.2's wash prediction. Server untouched (numeric-alphabet machinery).
- `speech-words` — **built; OFF-BRIEF** on two counts: word-level targets (rule 1, by name) and an ASR whose decoder embeds a language model against the brief's flat "no language models". Same quarantine as word-typing. The transferable knowledge: Web Speech API mechanics (on-device `processLocally` where available, may-use-network fallback surfaced in-UI), homophone-pruned lists for unambiguous ground truth, and the recognizer-on-trusted-gesture pattern (backend-less headless builds hard-crash on construction). The *compliant* voice path remains `voice-babble`: single-symbol targets, our own LM-free DSP recognizer.
